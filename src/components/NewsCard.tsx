...
export default function NewsCard({ article, isFeatured = false }: NewsCardProps) {
  ...
  return (
    <Card className={cn(
        "w-full flex flex-col overflow-hidden transition-all hover:shadow-xl", 
        isFeatured && "md:col-span-2 lg:grid lg:grid-cols-2 lg:gap-6",
        !isFeatured && "lg:col-span-1"
        )}>
      {article.imageUrl && (
        <Link href={articleUrl} className={cn(
